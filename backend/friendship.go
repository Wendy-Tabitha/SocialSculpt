...
func SendFriendRequest(w http.ResponseWriter, r *http.Request) {
	...
	_, err = db.Exec(`INSERT INTO friendships (requester_id, addressee_id, status)
...
func AcceptFriendRequest(w http.ResponseWriter, r *http.Request) {
	...
	res, err := db.Exec(`UPDATE friendships
        SET status='accepted'
...
func RemoveFriend(w http.ResponseWriter, r *http.Request) {
	...
	_, err := db.Exec(`DELETE FROM friendships
        WHERE (requester_id=$1 AND addressee_id=$2)
...
func FetchOnlineUsers(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := db.Query("SELECT id, nickname, gender FROM users WHERE id IN (SELECT user_id FROM sessions)")
	if err != nil {
